...
/*
  Convenience function to get info from barcode table
*/
CREATE OR REPLACE FUNCTION public.barcodes()
  RETURNS SETOF bc_symbols AS
$BODY$
DECLARE
  _row bc_symbols%ROWTYPE;
BEGIN

  FOR _row IN
    SELECT *
      FROM bc_symbols
     ORDER BY bc_symbol_name ASC
  LOOP
    RETURN NEXT _row;
  END LOOP;
  RETURN;
...
ALTER FUNCTION public.barcodes()
  OWNER TO postgres;
COMMENT ON FUNCTION public.barcodes() IS 'A table function that returns info on the barcodes supported by pgzint';
